...
| **Reward lane / F3 Fable re-pin** | T2AR2 binds integrated pairing receipt `e5351c3b…a259` into the re-sealed pending study and F3 ledger while preserving the unchanged 6,011-byte prompt. The preparation path verifies the receipt and every sealed file; its state is `withheld_pending_dispatch_verdict`, so it grants no call authority. See the [re-pin record](docs/operations/dual-orchestration/F3_FABLE_REPIN.md). This is configuration and plumbing only: zero candidate calls, standalone simulator commands, training, protected evaluations, or reward results. |
| **Experiment 004 / T2 reward-study pre-cycle** | The [expert-hold protocol](experiments/004_t2_reward_study/PROTOCOL.md) has a fail-closed `final_ready` admission state. Report hashes named files and replays traces; cross-manifest reconciliation is enforced by the final-ready gate. Baseline is registry-resolved; the candidate resolver is unit-tested and the canonical candidate is TBD. The execution manifest records launch-base provenance; the execution commit is verified at admission. Action-bound failure is interface-reachable; no production T2 trace producer has run. Report admission currently returns `study_not_final_ready`; dispatch, smoke, and training remain `NO-GO`. |
