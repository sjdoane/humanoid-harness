...
| **Experiment 004 / T2 reward-study pre-cycle** | The [expert-hold protocol](experiments/004_t2_reward_study/PROTOCOL.md) is now `final_ready`: baseline `eea2b6a9…1c5f`, recipe-derived candidate `c09e93dc…c123`, verified clean execution HEAD `28067291…3c97`, pairing key `ccef84c7…09a2`, and seal `89aff467…0624`. A well-formed synthetic report bound to this set passes admission; the superseded T2PAIRR1 seal still returns `study_not_final_ready`. See the [T2C1 result](docs/operations/dual-orchestration/T2C1_RESULT.md). These are artifact and interface checks only. Cycle 0, smoke, training, and any baseline measurement remain `NO-GO` pending review, resource reservation, and Samuel's explicit authorization. |
