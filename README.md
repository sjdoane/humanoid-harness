...
| **Experiment 003** | Cycles 0 and 1 ran on seeds `97001`-`97020`; both determinism replays matched `20/20`. The cycle-1 candidate had median speed MAE `3.2215 m/s`, fell in `20/20`, and spent the entire slow third in `medium` without reaching `simple`; it matched the switching baselines' fall count and had higher MAE. See the [cycle-1 report](experiments/003_composition_speed_profile/cycles/cycle_1/report_1.md), [cycle record](experiments/003_composition_speed_profile/cycles/cycle_1/cycle_record.md), and [cycle-2 prompt](experiments/003_composition_speed_profile/cycles/cycle_2/designer_prompt.md). This is `exploratory_oracle_cycle`; controller switching stands in for tracker following and supports no oracle-quality claim. |
