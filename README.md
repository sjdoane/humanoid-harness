...
| **Reward lane / F3 Fable re-pin** | The accepted initial-only F3 protocol is configured for Fable's `main` checkout: `fable-f3-prepare` prepares/ingests and `fable-f3-initial` is the sole candidate owner. The exact 1,773-byte tracking-only baseline and F2 identities are unchanged; the deterministic prompt is hash-bound and a second dispatch intent is refused. See the [re-pin record](docs/operations/dual-orchestration/F3_FABLE_REPIN.md). This is configuration and plumbing only: zero candidate calls, experimental simulator runs, training, protected evaluations, or reward results. |
| **Experiment 004 / T2 reward-study pre-cycle** | The [expert-hold protocol](experiments/004_t2_reward_study/PROTOCOL.md) freezes the no-transition oracle, expert-only four-environment training design, direct-state protected evaluator, five-policy paired analysis, matched-arm manifest, and an uncalled arm-invariant RNG adapter. The candidate reward and integrated trainer pairing receipt remain TBD, so execution is `NO-GO`. No F3 call, smoke, training, cohort, protected evaluation, or reward result exists; this is artifacts-and-interface evidence only. |
