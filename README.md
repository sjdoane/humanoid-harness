...
| **Reward lane / F3 Fable re-pin** | F3PINR2 derives call ID `e7f3ca01…0312a`, identity digest `b53841bc…0b375`, and the exact launcher scope from the committed T2PAIRR1 seal and baseline bytes. Both dispatch-facing documents are bound to the regenerated record by regression. The preparation state remains `withheld_pending_dispatch_verdict`; a narrow readback on the exact F3PINR2 commit and a separate dispatch verdict are still required. See the [re-pin record](docs/operations/dual-orchestration/F3_FABLE_REPIN.md). This is documentation and regression evidence only: zero candidate calls, standalone simulator commands, training, protected evaluations, or reward results. |
| **Experiment 004 / T2 reward-study pre-cycle** | The [expert-hold protocol](experiments/004_t2_reward_study/PROTOCOL.md) has a fail-closed `final_ready` admission state. Both common arm projections are type-validated and exact-byte compared. [T2PAIRR2](docs/operations/dual-orchestration/T2PAIRR2_RESULT.md) observes exact action, PPO-input, composition, reset, and RSI sinks against independent per-arm/per-slot expectations; five sink mutations and five route-specific disabled-seam checks pass. This is builder regression coverage pending narrow independent re-review, not runtime or behavior evidence. Baseline is registry-resolved; the canonical candidate is TBD. Report admission returns `study_not_final_ready`; dispatch, smoke, and training remain `NO-GO`. |
