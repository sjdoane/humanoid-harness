...
| **Experiment 004 / T2 reward-study pre-cycle** | The [expert-hold protocol](experiments/004_t2_reward_study/PROTOCOL.md) freezes the no-transition oracle, expert-only four-environment training design, direct-state protected evaluator, five-policy paired analysis, matched-arm manifest, and arm-invariant pairing key. The non-model-facing T2 seal re-verifies those artifacts and the tracking-only baseline without changing the prompt, but records `pairing_receipt: pending`; dispatch and execution remain `NO-GO`. No F3 call, smoke, training, cohort, protected evaluation, or reward result exists. |
