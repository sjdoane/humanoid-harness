...
| **Experiment 004 / T2 reward-study pre-cycle** | The [expert-hold protocol](experiments/004_t2_reward_study/PROTOCOL.md) has a fail-closed `final_ready` admission state. Both common arm projections are type-validated and exact-byte compared. A bounded regression runs both paired arms through the production PPO and controlled fake-runtime consumers, including a disabled-route mutation control; it is interface evidence only. Baseline is registry-resolved; the canonical candidate is TBD. Report admission returns `study_not_final_ready`; dispatch, smoke, and training remain `NO-GO`. |
