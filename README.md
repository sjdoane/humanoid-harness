...
| **Experiment 003 phase B / FT2R1** | Training admission now binds and replays the 68-fixture E1 receipt, exact optimizer membership, certified stock-COM task inputs, the reviewed smoke/cohort manifest, bounded actor archives, trace-derived protected metrics, strict raw designer provenance, and a disjoint calibration-receipt gate. The step-0 actor export remains byte-identical. See the [Phase B design and commands](experiments/003_composition_speed_profile/phase_b/DESIGN.md). This remains `interface_check`: no calibration run, disposable smoke, cohort, or behavioral evaluation ran. |
