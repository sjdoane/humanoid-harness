...
| **Experiment 003 phase B / FT1** | Interface slice implemented: canonical oracle/reward/checkpoint/run/report-v2 contracts; exact 708-D full-authority expert warm start and strict reloadable actor export; switch-only nearest-state phase transfer; composed `8 x 45` windows; and separate tracking/task/ignored-stock reward streams. E1 passed on `4` synthetic plus `64` SHA-ranked real fixtures, the nine-block static transfer ranges match the frozen survey, and a no-learning 200-step Humanoid-v5 smoke preserved expert actions bitwise. See the [Phase B design and receipts](experiments/003_composition_speed_profile/phase_b/DESIGN.md). This is `interface_check`, not training or behavioral evidence; FT2 remains unimplemented. |
