...
| **Experiment 003 phase B / FT1+FT2** | Interface implementation complete: canonical contracts; exact 708-D full-authority expert warm start; switch-only nearest-state phase transfer; composed `8 x 45` windows; tracking-only reward composition; CPU PPO worker with staged unfreeze and exact `50/50` composition/rehearsal accounting; supervised resource gates; deterministic final persistence; report-v2 science/telemetry separation; and train/evaluate CLI paths. E1 passed on `4` synthetic plus `64` SHA-ranked real fixtures, and the fake-runtime end-to-end exercises two seeds and all failure statuses. See the [Phase B design and commands](experiments/003_composition_speed_profile/phase_b/DESIGN.md). This remains `interface_check`: no disposable smoke, cohort, or behavioral evaluation ran. |
