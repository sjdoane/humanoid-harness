...
| **Experiment 003 phase B / FT2R2** | Training admission still binds the 68-fixture E1 receipt. Evaluator-owned trace replay, complete pre-construction evaluation lineage, atomic bounded episode supervision, non-scoring uncalibrated endpoints, rollout likelihood audits, manifest-complete reports, mailbox-bound reservations, bounded actor/checkpoint archives, production-only cohort authority, and the reviewed F2 formula registry are implemented. The step-0 actor export and Astra evaluator/parser bytes remain unchanged. See the [Phase B design and commands](experiments/003_composition_speed_profile/phase_b/DESIGN.md). This remains `interface_check`: no calibration run, disposable smoke, cohort, protected utility evaluation, or behavioral evaluation ran. FT2R3 still owns runtime isolation and mechanism-level negative repairs. |
