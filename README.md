...
| **Experiment 004 / T2 reward-study pre-cycle** | The [expert-hold protocol](experiments/004_t2_reward_study/PROTOCOL.md) now includes the `t2_reward_pairing/v1` shared-runtime seam and a source-bound fake-runtime `pairing_receipt_v1`. Exact study plus embedded arm-manifest bytes authorize the common key; the differing arm hashes remain execution identities. Action noise, minibatches, composition/reset/RSI schedules, and declared evaluation seed identities match at their indexed seams. The receipt is interface evidence only, and the pre-seam execution/study/F3 seals still require reviewed regeneration before admission; dispatch remains `NO-GO`. No F3 call, smoke, training, cohort, protected evaluation, or reward result exists. |
