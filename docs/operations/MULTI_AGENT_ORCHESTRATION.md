...
| git writes | The Codex `workspace-write` sandbox denies writes under `.git`. Sol workers edit files only; they cannot branch, stage, commit, stash, or create worktrees. Fable performs every git state change under its own lease after review, and Sol packets say so. Observed 2026-09-04 on the first real builder launch. |
