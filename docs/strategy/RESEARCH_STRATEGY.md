...
| next step | Study 020 (the paired saved-policy diagnostic, 2026-09-07 15:00Z) is done: initial policy 5 of 16 sampled falls, final 11 of 16, six survive-to-fall flips and none reversed, all falls at the rise hand-over or inside the crouch. Fable's ranked candidates for the next single declared test, Astra's call: an evaluation-only exploration-scale check of the same initial tensors with a smaller log-std under the same sixteen seeds; then, if the hand-over falls prove noise-driven at this scale, one training contrast on the 018 baseline's pins with that smaller initial log-std, read on sampled fall share and deterministic rows; reward-tail and oracle content contrasts after. Earlier context: Study 019 (Astra's declaration, guard 0.65 to 0.40 m) is done and negative: the control reproduced byte-exact, the candidate's prefix was exact, but the manipulation was not established (region entered at post-step phase 1.10 s on the same peak because the earlier switch handed a faster walking state to the crouch and progress per phase doubled) and the robot fell inside the crouch at 4.34 s; the lesson is that the clock-driven inside phase is not settable by the entry guard alone, so a progress-synchronized or state-conditioned inside phase is the general mechanism, needing its own declaration. After 019, Astra's stated preference (2026-09-07 12:38Z) is a bounded paired stochastic diagnostic of the retained 018 policies (exact initial and final tensors, 16 paired Gaussian noise seeds each, all original metrics plus hand-over states and downstream outcomes, descriptive only, under a new source-bound protocol once the 019 freeze ends), because only the first 512-transition rollout of A's training was collected under the exact initial policy, so the telemetry cannot bracket the initial stochastic fall share. Astra declared it as study 020 (pre-data draft 1664deb, 2026-09-07 13:49Z): the exact 018 initial and final tensors under the frozen 018 config and four-state finite-horizon runtime, both deterministic traces reproduced before any sampled episode, sixteen pinned 23-D Gaussian noise sequences shared pairwise with a balanced first-policy order, sampling from the unclipped mean with each policy's own log-std and training-equivalent clipping, raw episode return and fall indicator jointly primary, all eleven gates and a hand-over table secondary, no training and no change to the 018 verdict. Fable's review (13:51Z) found no claim-invalidating defect and pinned three implementation preconditions: reset-block parity without launching training, an eps-zero sampling path bit-identical to the deterministic predictor, and bound noise bytes. Result (2026-09-07 15:00Z, source 615124f, integrity all pass, action
path verified on every sampled action): the initial policy fell in 5 of 16
sampled episodes and the final policy in 11 of 16; six pairs flipped from
surviving to falling and none the other way; paired raw return differences
(final minus initial) had mean -1180 and median -451, and in four of the
five pairs where both survived the final policy still scored lower. Every
fall in both policies sat in the rise hand-over window or inside the crouch,
none in the walk before or the after window, and the falling episodes
entered the rise at 1.2 to 2.8 m/s against 0.3 to 1.1 m/s for survivors.
Reading (descriptive, one training seed, two fixed checkpoints, 16 paired
noise sequences): sampled exploration at the trainer's std already breaks
the initial policy at the rise hand-over in about a third of episodes, and
the 018 training left a policy that is worse under its own sampling as well
as deterministically; no safety certificate, no mechanism, no automatic
follow-on. A follow-up readback of the sixteen fallen episodes shows
the crouch-walk accelerating during the loop-boundary exit deferral: speed
at the 2.05 m guard 0.57 to 1.48 m/s, speed at the actual rise switch 1.19
to 2.81 m/s, against survivors switching at 0.34 to 1.14, and a slow inside
window existed before the fall in 14 of the 16. The deferral is the crouch
segment's exit_at_loop_boundary flag, a data-only field, so an immediate
rise exit at guard satisfaction is an oracle-only candidate (draft
validated through the firewall, held); Astra declared it as study 021 (pre-data protocol de88205, 2026-09-07
   15:54Z): one exact O7b control, then one immediate-exit zero-residual
   candidate, no training; the frame prefix may differ only in the recomputed
   crouch segment identity at the existing before-to-inside transition, with
   the numeric prefix exact through the changed command; screen rows are
   manipulation, survival, composition, exposure, recovery and tracking, with
   the eleven gates reported unchanged, and Fable's single data-only proposal
   binds the retained O7b feedback packet; Fable reviewed the protocol with no
   claim-invalidating defect, with the caveat that a deterministic pass or fall does not transfer
   to the sampled regime; Fable's ranking: zero-residual immediate-
exit probe, then the sampled diagnostic of the initial policy under that
oracle, then the scale test, then any training. Fable's priority sketch for reading that diagnostic (16 rollouts per policy; falls and raw-return paired differences jointly primary; descriptive, not decision rules, with Astra's limits accepted): a high initial fall share would raise the priority of a trainer-side exploration contrast for this exact policy and reset; a low initial and high final share would raise one trainer-side contrast on the 018 baseline's pins; both low would raise the course-signal study without certifying safety or naming a mechanism; a high initial and low final share would pose a sampled-versus-mean deployment question under its own declaration with the 018 verdict unchanged; a deterministic-parity failure stops everything. The hand-over table (state at rise and after entry with downstream outcome) is reported and cannot establish dominance or separate speed and heading from history; every rival test (exploration, trainer, oracle, reward) needs its own declaration, prioritized by effect size and uncertainty, with the two-knob composition demonstration as the main priority. Deferred behind it, as separately declared and not yet numbered studies if worthwhile: the course-signal availability study, in Astra's proposed form a single-kernel contrast that swaps the lateral and heading Gaussian terms for Cauchy tails at unchanged scales and weights (Fable's audit on the retained traces: the swap restores a persistent heading gradient at large heading errors but only a marginal lateral one, so the primary readout should be the after-window turning rate against an r1 replication that also serves as the determinism floor) that trains on the 018 baseline's exact pins with a non-saturating heading and lateral reward signal (new recipe fields, data-only contrast), read out on the after-window turning rate against the zero-residual 0.084 rad/s and on the training fall trend; if steering does not appear with the signal active, the lateral problem returns to reference content. For depth, the reference remains the suspect: a consistency control from a retained executed crouch (option B), then a contact-aware repaired reference ladder (option A), both labeled reference-supply adapter work, never oracle composition. The frozen 0.50 m dip gate is preserved with its uncertainty reported; Samuel is informed, and continuing to study the gate needs no new decision from him. |
...
6. Study 018 (Astra's decision, 2026-09-07 09:05Z, contract reviewed by
   ...
   hypothesis carrying the post-step manipulation definition). Result
   (2026-09-07 13:35Z, source 9d7396a): the fresh control reproduced the
   retained O7b byte-exact and the candidate's prefix was exact through the
   changed command, but the manipulation was not established: the earlier
   switch caught the walk while accelerating (0.77 m/s at the switch against
   0.58), the crouch never slowed (about 1.0 to 1.2 m/s against 0.5 to 0.8),
   ...
   state-conditioned inside phase is the general mechanism and needs its
   own declaration. No guard sweep. Astra's sealed score agrees row for
