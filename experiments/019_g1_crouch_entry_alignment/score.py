...
_DEVELOPMENT_GATES = {
    "finish_reached", "full_horizon_without_fall", "inside_mean_speed_target",
    "inside_posture_compliance", "inside_posture_dip", "joint_position_rmse_p95",
    "maximum_lateral_error", "mean_speed_error", "minimum_inside_samples",
    "region_entry_and_exit_observed", "roll_pitch_rmse_p95",
}
...
def _validate_common(inputs: dict, *, extra: set[str]) -> dict[str, object]:
    fields = {
        "schema_version", "scorer_sha256", "protocol_sha256", "dependency_sha256",
        "source_commit", "source_tree_sha256", "source_file_count", "coordination_root",
    }
    if (
    ...
    if _sha256(_SCORER_PATH.read_bytes()) != scorer or _sha256(_PROTOCOL_PATH.read_bytes()) != protocol:
    ...
    if type(dependencies) is not dict or set(dependencies) != set(paths) or any(
        _digest(dependencies[name], length=64, field=f"{name} SHA-256")
        != _sha256(path.read_bytes()) for name, path in paths.items()
    ):
    ...
    if type(count) is not int or count < 1 or not raw_root.is_absolute() or root != raw_root or not root.is_dir():
        raise ValueError("source count or coordination root differs")
    common = {
        "scorer_sha256": scorer, "protocol_sha256": protocol,
        ...
        "source_file_count": count, "coordination_root": root,
    }
...
def _retained(inputs: dict, common: dict[str, object]) -> Any:
    ...
    return _run_pins(inputs["retained"], common, config_sha256=RETAINED_BINDING["config_sha256"], fresh=False)


def _verify_run(pins: Any, feedback_root: Path, *, tree: str, count: int) -> dict:
    run = shared._verify_run(
        pins, feedback_root, expected_source_tree_sha256=tree,
...
def verify_config(control: Any, candidate: Any) -> dict[str, object]:
    ...
    if (
        ...
        or raw.get("mode") != "probe" or raw.get("training_steps") != 0 or raw.get("seed") != SEED
    ):
    ...
    if (
        type(first) is not list or type(second) is not list or len(first) != 3 or len(second) != 3
        or first[0].get("from") != "before" or first[0].get("to") != "inside"
        or first[0].get("guard") != "x_travelled >= 0.65"
        or second[0].get("guard") != "x_travelled >= 0.40"
        or type(candidate_id) is not str or not candidate_id or candidate_id == expected["oracle"]["oracle_id"]
    ):
    ...
    return {"control_oracle_id": parent_id, "candidate_oracle_id": candidate_id,
            "control_guard": "x_travelled >= 0.65", "candidate_guard": "x_travelled >= 0.40"}


def replay_native_loop(config: Any, run: dict, *, diagnostic_indices: set[int] | None = None) -> dict:
    ...
    for index, row in enumerate(frames):
        ...
        previous_segment, source_phase = config.segments[previous_behavior], oracle._phase(index).clone()
        ...
        if previous_state == "before" and inputs.projection.progress_m >= entry_threshold and first_entry_guard is None:
            first_entry_guard = {
                "action_index": index, "command_number_1_based": index + 1,
        ...
        if previous_state == "inside" and inputs.projection.progress_m >= 2.05 and first_exit_guard is None:
            first_exit_guard = {
                "action_index": index, "command_number_1_based": index + 1,
                "progress_m": inputs.projection.progress_m,
                "source_phase_seconds": previous_segment.reported_phase(source_phase),
        ...
        if window.shape != (REFERENCE_HORIZON, REFERENCE_FRAME_DIM) or not np.array_equal(endpoint, references[index]):
        ...
        if command.transition is not None:
            transitions.append({
                ...
                "source_target_height_m": float(source_target[0]),
                "poststep_target_height_m": float(endpoint[0]), **dict(command.transition),
            })
    stacked_windows, stacked_endpoints = np.stack(windows), np.stack(endpoints)
    entry = next((row for row in transitions if row["from_state"] == "before" and row["to_state"] == "inside"), None)
    rise = next((row for row in transitions if row["from_state"] == "inside" and row["to_state"] == "rise"), None)
    entry_dispatch = None if first_entry_guard is None or entry is None else {
        "guard": entry_rule, "first_guard_satisfaction": first_entry_guard,
        "actual_switch": entry,
        "deferred_action_count": entry["action_index"] - first_entry_guard["action_index"],
        "elapsed_deferral_seconds": (entry["action_index"] - first_entry_guard["action_index"]) * 0.02,
    }
    exit_deferral = None if first_exit_guard is None or rise is None else {
        "first_guard_satisfaction": first_exit_guard, "actual_switch": rise,
        "deferred_action_count": rise["action_index"] - first_exit_guard["action_index"],
        "elapsed_deferral_seconds": (rise["action_index"] - first_exit_guard["action_index"]) * 0.02,
    }
    return {
        ...
        "all_retained_actions_replayed": True, "command_count": len(frames),
        ...
        "transitions": transitions, "entry_boundary_dispatch": entry_dispatch,
        "inside_exit_boundary_deferral": exit_deferral,
        "diagnostic_poststep_targets": observed,
    }


def verify_prefix(control: dict, candidate: dict, control_replay: dict, candidate_replay: dict) -> dict:
    ...
    if (
        control_entry is None or candidate_entry is None
        or control_entry["action_index"] != 92 or candidate_entry["action_index"] != 71
        or control_entry.get("selected_phase_seconds") != 0.0
        or candidate_entry.get("selected_phase_seconds") != 0.0
        or type(control_dispatch) is not dict or type(candidate_dispatch) is not dict
    ...
    for name, count in (("composite_raw_action", 71), ("residual_action", 71),
                        ("current_reference", 71), ("qpos", 72), ("qvel", 72)):
        if not np.array_equal(control["trajectory"][name][:count], candidate["trajectory"][name][:count]):
    ...
    if control["frames"][71] == candidate["frames"][71] or np.array_equal(
        control["trajectory"]["current_reference"][71], candidate["trajectory"]["current_reference"][71]
    ):
        raise ValueError("preregistered first changed command is not command 72")
    return {"equal_action_and_frame_rows": 71, "equal_state_boundaries": 72,
            "first_changed_action_index": 71, "first_changed_command_number_1_based": 72}
...
def _objective(run: dict) -> dict:
    ...
    if (
        type(gates) is not dict or set(gates) != _DEVELOPMENT_GATES
...
def _reset_and_base(control: dict, candidate: dict, control_config: Any, candidate_config: Any) -> dict:
    evaluations = [decode_json_object(run["outputs"]["zero_residual_evaluation.json"], source="zero evaluation") for run in (control, candidate)]
    ...
    if resets[0].get("oracle_sha256") != control_config.program.sha256 or resets[1].get("oracle_sha256") != candidate_config.program.sha256:
        raise ValueError("paired reset oracle identities differ")
    if {k: v for k, v in resets[0].items() if k != "oracle_sha256"} != {k: v for k, v in resets[1].items() if k != "oracle_sha256"}:
    ...
    return {"equal_beyond_oracle_identity": True, "initial_qpos_qvel_equal": True,
...
def _diagnostics(run: dict, config: Any) -> dict[str, object]:
    ...
    if signed_after and abs(signed_heading["after_peak_heading_signed_unwrapped_rad"]) != heading["after_heading_max_abs_unwrapped_rad"]:
        raise ValueError("signed after-heading reconstruction differs")
    entry_target = replay["diagnostic_poststep_targets"].get(str(entry)) if entry is not None else None
    rise = next((row for row in replay["transitions"] if row["to_state"] == "rise"), None)
    return {
        "objective": objective, "visits": visits, "contacts": contacts, "replay": replay,
        "heading": heading,
        "measurements": {
            "minimum_physical_region_height_m": objective["region"]["minimum_root_height_m"],
            "physical_region_posture_compliant_fraction": objective["region"]["posture_compliant_fraction"],
            "inside_mean_speed_target_deviation_m_s": objective["speed"]["inside_mean_speed_target_deviation_m_s"],
            ...
            "first_physical_entry_executed_mode": None if entry is None else run["frames"][entry]["executed_mode"],
            "first_physical_entry_pre_action_phase_seconds": None if entry is None else run["frames"][entry]["executed_phase_seconds"],
...
def candidate_criteria(diagnostics: dict) -> tuple[dict[str, bool], dict[str, bool]]:
    ...
    manipulation = {
        "first_physical_entry_executes_inside": measures["first_physical_entry_executed_mode"] == "inside",
        "entry_poststep_phase_in_1p45_to_1p80_s": target is not None and 1.45 <= target["poststep_phase_seconds"] <= 1.80,
        "entry_poststep_target_height_below_0p50_m": target is not None and target["poststep_target_height_m"] < 0.50,
    }
    oracle, tracking, region = objective["oracle_diagnostics"], objective["tracking"], objective["region"]
    counts = oracle["executed_mode_counts"]
    if type(counts) is not dict or set(counts) - _MODES or any(type(v) is not int or v < 0 for v in counts.values()):
    ...
    rows = {
        ...
        "survival": objective["duration_seconds"] == 20.0 and objective["fall_count"] == 0 and contacts["nonfoot_ground_contact_action_count"] == 0,
        "composition": oracle["observed_switch_count"] == 3 and set(counts) == _MODES and all(counts.values()),
        "exposure": region["entry_observed"] is True and region["exit_observed_after_entry"] is True and objective["finish_condition_observed"] is True and visits["all_visit_sample_count"] >= 25,
        "recovery_guardrail": visits["after_reentry_sample_count"] == 0,
        "tracking": tracking["joint_position_rmse_rad_p95"] <= 0.35 and tracking["roll_pitch_rmse_rad_p95"] <= 0.25,
    }
    return rows, manipulation


def _fresh(inputs: dict, common: dict[str, object], name: str, config_sha256: str) -> tuple[Any, dict, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"study019-{name}-", dir=temp_parent) as raw:
        run = _verify_run(pins, Path(raw) / "feedback", tree=common["source_tree_sha256"], count=common["source_file_count"])
...
def score_control(inputs: dict) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="study019-retained-", dir=temp_parent) as raw:
        retained = _verify_run(retained_pins, Path(raw) / "feedback", tree=RETAINED_SOURCE_TREE_SHA256, count=RETAINED_SOURCE_FILE_COUNT)
    ...
    _objective(control)
    return {
        "schema_version": 1, "study": "019",
        "artifact": "gmt_g1_study019_control_verification/v1",
        **{name: common[name] for name in ("scorer_sha256", "protocol_sha256", "dependency_sha256", "source_commit", "source_tree_sha256", "source_file_count")},
        "retained_binding": inputs["retained"], "control_binding": inputs["control"],
        ...
        "loop_runtime": loop, "native_reference_replay": replay,
...
def _control_receipt(binding: object, common: dict[str, object], inputs: dict) -> dict:
    ...
    exact = all(receipt.get(name) == common[name] for name in (
        "scorer_sha256", "protocol_sha256", "dependency_sha256", "source_commit",
    ...
    if (
        ...
        or receipt.get("study") != "019" or not exact
        ...
        or receipt.get("fresh_feedback_parity", {}).get("content_equal_except_source_manifest_sha256") is not True
        or receipt["fresh_feedback_parity"].get("retained_feedback_sha256") != PARENT_FEEDBACK_SHA256
        or fresh_feedback == PARENT_FEEDBACK_SHA256
        or receipt["fresh_feedback_parity"].get("manifest_bound_feedback_sha256_different") is not True
    ):
...
def _proposal(inputs: dict, control_config: Any, candidate_config: Any, control: dict) -> dict:
    ...
    if (
        proposal.get("factor") != "oracle" or feedback_sha != PARENT_FEEDBACK_SHA256
    ...
    return {"path": str(proposal_path), "sha256": proposal_sha,
            "proposal_id": proposal["proposal_id"], "feedback_path": str(feedback_path),
            "feedback_sha256": feedback_sha, "hypothesis": hypothesis}


def score_pair(inputs: dict) -> dict[str, object]:
    extra = {"retained", "control", "candidate", "candidate_config_sha256",
             "control_verification", "proposal", "parent_feedback"}
    ...
    control_pins, control, control_config = _fresh(inputs, common, "control", RETAINED_BINDING["config_sha256"])
    candidate_pins, candidate, candidate_config = _fresh(inputs, common, "candidate", candidate_hash)
    if (
    ...
    loop = {"control": study017.verify_loop_manifest(control),
    ...
    prefix = verify_prefix(control, candidate, control_diagnostics["replay"], candidate_diagnostics["replay"])
    ...
    return {
        ...
        **{name: common[name] for name in ("scorer_sha256", "protocol_sha256", "dependency_sha256", "source_commit", "source_tree_sha256", "source_file_count")},
        ...
        "proposal": proposal, "config_change": config_change,
        "fixed_resource_inputs_equal": True,
        "fixed_resource_inputs": control["resource_fixed_inputs"], "loop_runtime": loop,
        "reset_and_base_state": reset, "prefix_integrity": prefix,
        "candidate_criteria": rows, "manipulation_checks": manipulation,
        "study_screen_passed": all(rows.values()),
        "measurements": {name: value["measurements"] for name, value in (
            ("control", control_diagnostics), ("candidate", candidate_diagnostics))},
        "native_reference_replay": {name: value["replay"] for name, value in (
            ("control", control_diagnostics), ("candidate", candidate_diagnostics))},
        "physical_region_visits": {name: value["visits"] for name, value in (
            ("control", control_diagnostics), ("candidate", candidate_diagnostics))},
        "recorded_ground_contacts": {name: value["contacts"] for name, value in (
            ("control", control_diagnostics), ("candidate", candidate_diagnostics))},
        "original_development_gate_results": {name: value["objective"]["development_gate_results"] for name, value in (
            ("control", control_diagnostics), ("candidate", candidate_diagnostics))},
        "raw_objectives": {name: value["objective"] for name, value in (
            ("control", control_diagnostics), ("candidate", candidate_diagnostics))},
        "claim_scope": "one_seed_zero_training_oracle_screen_not_causal_isolation_or_generalization",
        "claim_limits": ["windows_are_reconstructed_not_directly_retained", "no_training",
